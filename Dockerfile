...
RUN tar -xvf projectfiles.tar \
    && dotnet restore "src/Members.API/Members.API.csproj" --configfile ./NuGet.docker.config /p:DockerBuild=true -r linux-musl-x64 \
    && rm projectfiles.tar  # Remove the tar file to reduce image size
COPY . .
RUN dotnet build "src/Members.API/Members.API.csproj" -c $BUILD_CONFIGURATION --no-restore /p:DockerBuild=true -r linux-musl-x64

FROM build AS publish
ARG BUILD_CONFIGURATION
RUN dotnet publish "src/Members.API/Members.API.csproj" -c $BUILD_CONFIGURATION --no-build -o /app/publish /p:UseAppHost=true /p:DockerBuild=true -r linux-musl-x64
