...
namespace Members.Infrastructure.Repositories
{
        ...
        : RepositoryBase<RoleRepository, MembersDbContext>(aContext, aLogger), IRoleRepository, ISortRepository
    {
        ...
        public async Task<IHttpResult<IEnumerable<Role>>> GetByIdListAsync(IEnumerable<ulong> aRoleIdList, CancellationToken aCancellationToken = default)
        => await TryQueryAsync(async (aCancellationToken)
            => await _context.Roles
                .Where(role => aRoleIdList.Contains(role.Id))
        ...
        public async Task<IHttpResult<Role>> AddAsync(Role aNewRole, CancellationToken aCancellationToken = default)
        => await TryCommandAsync(async (aCancellationToken) =>
        {
            ...
            //};
            var lNewRole = (await _context.Roles.AddAsync(aNewRole, aCancellationToken)).Entity;
            return lNewRole;
        }, aCancellationToken);
